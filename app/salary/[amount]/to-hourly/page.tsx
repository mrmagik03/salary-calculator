...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Convert ${formatCurrency(amount, 0)} a year to hourly pay. See monthly pay, biweekly pay, and a quick salary breakdown.`,
    alternates: { canonical: `${SITE_URL}/salary/${amount}/to-hourly` },
  };
}
...
export default async function SalaryToHourlyPage({ params }: PageProps) {
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `How much is ${salaryLabel} a year per hour?`,
        acceptedAnswer: { "@type": "Answer", text: `${salaryLabel} per year is about ${hourlyLabel} per hour based on a 40-hour work week.` },
      },
      {
        "@type": "Question",
        name: `What is ${salaryLabel} annually per month?`,
    ...
    <SiteShell>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 md:px-6">
        ...
        <section className="rounded-2xl border border-neutral-800 bg-neutral-900 p-8 shadow-sm">
          <h1 className="text-3xl font-bold tracking-tight md:text-5xl">{salaryLabel} a Year is How Much an Hour?</h1>
          <p className="mt-4 max-w-3xl text-neutral-300">
            A salary of <strong>{salaryLabel}</strong> per year works out to about <strong>{hourlyLabel} per hour</strong>, assuming a 40-hour work week and 52 weeks per year.
          </p>
        </section>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
          <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6 shadow-sm">
            ...
            <div className="mt-6 overflow-hidden rounded-xl border border-neutral-800">
              ...
              <div className="grid grid-cols-2 border-b border-neutral-800 px-4 py-3 text-sm"><div className="text-neutral-400">{salaryLabel} per year</div><div>{hourlyLabel} per hour</div></div>
              ...
              <div className="grid grid-cols-2 px-4 py-3 text-sm"><div className="text-neutral-400">Per biweekly paycheck</div><div>{biweeklyLabel}</div></div>
            </div>
          </div>
          <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6 shadow-sm">
